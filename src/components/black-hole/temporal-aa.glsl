...
{
    float discRadius = uDiscRadius;
    float discWidth = uDiscWidth;
...
{
    vec3 angle = vec3(0.08, 1.05, -0.45);
...
void WarpSpace(inout vec3 eyevec, inout vec3 raypos)
{
    ...
    eyevec = normalize(eyevec + singularityVector * warpFactor * uWarpAmount / float(ITERATIONS));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    #ifdef TEMPORAL_AA
    uveye.x += (rand(uv + sin(iTime * 1.0)) / iResolution.x) * uTemporalJitter;
    uveye.y += (rand(uv + 1.0 + sin(iTime * 1.0)) / iResolution.y) * uTemporalJitter;
    #endif

    vec2 offsetUV = uveye + uViewOffset;
    vec3 eyevec = normalize(vec3((offsetUV * 2.0 - 1.0) * vec2(aspect, 1.0), uEyeDepth));
    vec3 eyepos = vec3(-1.2, 0.8, -10.0);
    ...
    float blendWeight = uTemporalBlend;
